...
def query_server(token, password, server=SERVER_ADDRESS):
    ...
    """
    res = requests.post(SERVER_ADDRESS, data={
...
def setup_ui():
    ...
    a = Label(window, text='Enter your supersecret credentials',
              name='main_text').grid(columnspan=4)
    b = Label(window, text='Token:').grid(row=1)
    token_entry = Entry(window, name='token').grid(
        row=1, column=1, columnspan=3)
    c = Label(window, text='Password:').grid(row=2)
    pass_entry = Entry(window, name='password').grid(
        row=2, column=1, columnspan=3)
    submit = Button(window, text='Submit', command=lambda: click(
        window)).grid(row=3, columnspan=2)
    window.mainloop()
